...
| **Landing** | Every landmark, the biggest trees and boulders offer a perch: a soft ring marks the one ahead. Slow to under 24 m/s (X brakes) and glide onto it; the bird flares, reaches with its legs and settles with its wings folded along its flanks. Perched, drag to look around, watch it look about and preen, listen (the airflow gives way to a breeze in the leaves and songbirds answering each other); boost recovers. **Space** hops off |
...
**Settings** (from the start screen, the pause menu or the in-flight button) is a scrollable dialog that always sits above the other overlays: it takes focus when it opens, Tab stays inside it, Esc closes it and focus returns to the control that opened it. It holds the bird picker (four illustrated cards: **Eagle**, **Gull**, **Swallow**, **Owl**, each a smooth lathed body with three-segment wings whose plates overlap at the hinges under a row of coverts, a fanned tail and crisp species marks, and each with its own silhouette, colours, wingbeat and handling: speed, agility, glide and flap power differ by up to about a third, the controls stay the same), the music style (**Sunny stroll**, **Meadow waltz**, **Island breeze**, **Calm pad**, **Off**: procedural tunes that start playing the moment you pick one), the soundscape mixer (master, nature & wind, music, wings & discoveries, mute), the ambient life density (**Off**, **Subtle**, **Lively**) and the **Changing skies** toggle.
...
- A procedural Web Audio soundscape with separate buses: a bright, gusting band of air plus a low rush that only grows with speed, soft wingbeats, water that fades in near shores and lakes, sparse daytime birdsong (busier, in call-and-answer phrases, over a leafy breeze while the bird sits on a perch), and a music box that composes short cheerful tunes on the fly (major keys, pentatonic melodies, four-chord loops, plucked or mallet leads, light percussion) in three styles plus a calm pad. No audio files are shipped.
- Ambient life: small flocks circling over the land, deer resting in meadows and uplands, ducks swimming on ponds, hot-air balloons drifting above gentle country, sailboats on open water and leaping fish on any water deeper than a few metres. Encounters are deterministic per seed, prepared one habitat per frame, animated on the GPU, purely decorative (no colliders), and capped per density setting. Deer coats vary from reddish to grey-brown, about half the ducks are brown-headed hens, and flock birds differ in shade. Animals notice the bird: a flock bursts outward and closes up again when you fly through it, deer lift their heads and trot off when you pass low, ducks paddle away and leave ripples. Reactions are planned per group on a quarter-second tick (a few groups at a time, with a cooldown, run paths checked against the terrain so nothing runs into a hill or onto the shore) and played on the GPU, so they cost no per-frame work.
- Ground cover with a dozen tile kinds: meadow, dry and coastal grass, reeds, ferns on the woodland floor, tall grass with seed heads, and five wildflowers (poppies, daisies, lupines, cornflowers, buttercups) that grow in colour-coherent fields, one dominant flower per zone of about 170 m with organic borders; every tuft carries its own tint, so grass yellows in dry zones and each flower spans a few hues. Boulders on alpine, arid and upland slopes, and slowly changing skies: haze and cloud cover drift over minutes, and clouds come in three sizes from small fair-weather puffs to towering masses, so the same route never looks quite the same.
- A brook meanders down the mist-valley floor into its lake, fed by a waterfall off the head wall: both are flowing-foam ribbons laid on the terrain (nothing is carved, so saves and collision are unchanged), spray hangs at the foot of the fall and its rush grows on the water bus as you approach.
- Terrain colour follows the climate: dry ground goes olive, moist ground deeper and bluer, warm regions warmer; uplands carry heather patches and a soft wildflower blush that follows the flower zones; rock drifts by region from grey to ochre to dark basalt, with scree speckling the soil band. Terrain shading is procedural per pixel: rock on steep faces with strata and cracks, a dusty soil band where grass thins out before bare rock, snow that collects on shelves with a wide, noisy edge, wet banks, distance-aware grain (the noise comes from one baked, mipmapped texture rather than sine hashes per pixel) whose mid-field mottle fades out by about 2 km so far slopes read as broad masses. Aerial perspective is applied in the terrain and vegetation shaders ahead of the scene fog: from a few hundred metres out the lit colour loses saturation and contrast toward the sky tint, so far ranges sit behind the near ground rather than competing with it. The sky's horizon band belongs to the sun side when the sun is low, dusk has its own rose-and-violet palette rather than replaying dawn, sunrise keeps a cool sky light so shadows read blue against the warm faces, the bird is lit with a wrapped diffuse so its shaded side keeps its colour, and it casts a soft contact shadow on its perch. Tree trunks and boulder bases darken toward the ground. Thirteen vegetation species (oak, pine, birch, palm, cactus, deadwood, shrub, willow, boulder, maple, cypress, blossom, fir) are built from leaf clusters with soft, low-frequency lumps and a lift toward the sunlit top (80 faces each: 320-face clusters were tried and cost 4-7 fps on an integrated GPU) and 10-sided conifer tiers; every tree and impostor carries a crown tint of its own, with regional zones about 400 m across where some deciduous trees turn yellow and red, maples run from gold to deep red, blossom trees from white to rose and conifers from yellow-green to blue-green. Trees have trunks, branches and layered crowns within about 700 m of the bird, billboard impostors further out, and grass tufts in the near field; trees are placed at a fixed density on every preset so collision is identical, and their shadows are cast by two instanced unit shapes per chunk (a blob and a cone, scaled per species) rather than the full crowns, so the caster pass costs two draw calls per chunk. Vegetation and impostors dissolve in and out over 0.7 s (a screen-door fade) instead of popping, terrain chunks geomorph between detail levels over 0.6 s (each vertex carries the coarser surface it came from, and collision follows the blended surface), the coarsest chunks rise out of the far shell the same way, and chunks are requested ahead along the flight direction so the detailed tier is usually in place before it comes into view.
...
- Ambient animals, balloons and boats are low-poly instanced decorations on fixed circles, not autonomous agents (no pathfinding, no flocking between groups). Their reactions to the bird are a small per-group state machine (idle, alert, evade, recover) driven by a threat estimate from distance, closing speed, height and speed, played on the GPU: flocks swing aside of the approach and re-form, deer turn their heads and then run a checked corridor on two-segment legs, ducks paddle in a line and swim off with a ripple trail (or flutter-run across the water, wings beating, when dived at), fish dive when the bird touches the water nearby. Shoals are never placed within a cell of a duck pond, so fish do not leap among the ducks. "Changing skies" varies haze and cloud cover but there is no rain or ring course. The wind is a fixed direction used for lift, trees and clouds, not a simulated field.
